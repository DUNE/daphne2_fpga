...
architecture trig_xc_arch of trig_xc is
    ...
    signal st_xc_filt_dout, st_xc_filt_dout_reg36, st_xc_mov_mean, st_xc_mov_mean_sub: std_logic_vector(13 downto 0);
...
------------------------------------------------------------------------------------------------------------------------------
    ...
    port map (
        ...
        dout => st_xc_mov_mean_sub,
        mov_mean => st_xc_mov_mean
    );
        ...
        if rising_edge(clock) then
            if (reset='1') then
                ...
                if (triggered_core='1') then
                    trigsample_reg <= din_delayed; 
                end if;
            end if;
        end if;
    end process xc_trigsample_proc;

    dout <= st_xc_filt_dout_reg36; -- delayed signal
    dout_mm <= st_xc_mov_mean; -- moving average of the signal synchronized with it
